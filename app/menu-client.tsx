    ...
    <main className="min-h-screen bg-background pb-20 text-foreground">
      <section className="hero relative overflow-hidden px-5 pb-7 pt-6 text-white">
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="mb-9 flex items-center justify-between">
            ...
            <div className="relative"><button aria-label={`${favorites.length} favori ürün`} className="grid h-11 w-11 place-items-center rounded-full border border-white/15 bg-black/10 backdrop-blur"><Heart size={19} fill={favorites.length ? 'currentColor' : 'none'} /></button>{favorites.length > 0 && <span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-[#40d9a5] px-1 text-[10px] font-black text-[#0f2b25]">{favorites.length}</span>}</div>
          </div>
      ...
      <section className="mx-auto max-w-4xl px-5 pt-7">
        ...
        {results.length ? <div className="grid gap-3 sm:grid-cols-2">
          {results.map((item) => <article key={item.id} className="menu-card group"><div className="flex min-w-0 flex-1 gap-3"><div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-[radial-gradient(circle_at_top_right,#77e8bf_0,transparent_40%),linear-gradient(145deg,#eef8f4,#d8ede5)] text-2xl" aria-hidden>{item.icon}</div><div className="min-w-0"><div className="flex items-center gap-2"><h3 className="font-semibold leading-5">{item.name}</h3>{item.popular && <Sparkles size={14} className="shrink-0 text-[#e7a72f]" aria-label="Popüler" />}</div><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{item.description ?? item.categoryName}</p></div></div><div className="flex shrink-0 flex-col items-end justify-between gap-2"><button onClick={() => toggleFavorite(item.id)} aria-label={`${item.name} ürününü favorilere ${favorites.includes(item.id) ? 'çıkar' : 'ekle'}`} className="text-[#789087] transition hover:text-[#159875]"><Heart size={18} fill={favorites.includes(item.id) ? '#159875' : 'none'} className={favorites.includes(item.id) ? 'text-[#159875]' : ''} /></button><strong className={`text-right text-sm ${item.price ? 'text-[#117b60]' : 'max-w-16 text-[10px] leading-3 text-[#82928d]'}`}>{item.price ? money.format(item.price) : 'Fiyat için sorunuz'}</strong></div></article>)}
